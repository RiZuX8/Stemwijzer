...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Get statement ID from URL parameter
    const urlParams = new URLSearchParams(window.location.search);
    const statementId = urlParams.get('id');

    // Fetch and populate existing statement data using getStatement function
    getStatement(statementId)
        .then(statement => {
            document.getElementById('title').value = statement.name;
            document.getElementById('description').value = statement.description;
            document.getElementById('x-as').value = statement.xValue;
            document.getElementById('y-as').value = statement.yValue;
            document.getElementById('priority').checked = statement.priority === 1;
        })
        .catch(error => {
            errorMessage.textContent = 'Kon stelling niet laden: ' + error.message;
        });
    ...
    form.addEventListener('submit', async function(e) {
        ...
        updateStatement(statementId, title, description, xValue, yValue, priority)
        .then(response => {
            errorMessage.textContent = 'Stelling succesvol bijgewerkt';
            setTimeout(() => window.location.href = 'statements.php', 1000);
        })
        .catch(error => {
            errorMessage.textContent = 'Fout bij bijwerken: ' + error.message;
        });
